...
const CloudinessCard = withTheme((props: { forecastData: IForecastWeather[], theme: Theme }) => {
	...
	const sanitisedCloudData = props.forecastData.map(weather => {
		return {
			time: new Date(weather.dt_txt).toLocaleString('en-AU', { month: 'long', day: '2-digit', hour: 'numeric' }),
	...
	return (
		<InfoCard title='Weekly Cloudiness' style={{ marginTop: '10px' }}>
			<AreaChart data={sanitisedCloudData} height={80} width={220} margin={{ top: 12 }}>
				<Tooltip />
				<XAxis dataKey='time' hide={true} />
				<Area dataKey='clouds' fillOpacity={1} stroke={props.theme.secondColor} fill={props.theme.shallowSecondColor} />
			</AreaChart>
		</InfoCard>
	)
})
